package com.jeetx.common.exception;
...
import com.jeetx.util.oConvertUtil;
...
public class GlobalExceptionResolver implements HandlerExceptionResolver {
	...
	private void logDb(Throwable ex) {
		...
		//logService.addLog(exceptionMessage, LOG_OPT,LOG_LEVEL);
	}
	/**
	 * ajax异常处理并返回.
	 * @param request
	...
	private ModelAndView processAjax(HttpServletRequest request,
		...
		json.setSuccess(false);
		if("AuthorizationException".equals(deepestException.getClass().getSimpleName())) {
			json.setMsg("抱歉，没有权限访问！");
		}else {
			json.setMsg(deepestException.getClass().getSimpleName());
		}

		PrintWriter pw = null;
		try {
			pw=response.getWriter();
			pw.write(JsonUtil.bean2json(json));
			pw.flush();
		} catch (IOException e) {
			e.printStackTrace();
		}finally{
	...
	private ModelAndView processNotAjax(HttpServletRequest request,
		...
		if("AuthorizationException".equals(ex.getClass().getSimpleName())) {
			return new ModelAndView("common/unauthorized", model);
		}else {
